...
def generate_app_password():
    ...
    # Must match generate_app_password() in mynode_functions.sh.
...
def reset_lnbits_super_user_pwd():
    """
    Gives the LNbits super_user a new password.

    The password itself is generated by pre_lnbits.sh on the next start, which is also where
    a fresh install gets one. Clearing the stored hash is what tells it to do so. The new
    password is shown on the LNbits app page.
    """
    try:
        super_user_id, super_user_username = fetch_super_user_info()
        print(f"LNbits super_user password reset")
        print(f"super_user ID: {super_user_id}")
        print(f"super_user username: {super_user_username}")

        # Keyed on running, not enabled: the password is only replaced when pre_lnbits.sh runs
        # again, so a running service has to be restarted either way
        was_active = is_service_active("lnbits")
        if was_active:
            stop_service("lnbits")

        delete_file("/mnt/hdd/mynode/lnbits/.app_password")

        db = sqlite3.connect("/mnt/hdd/mynode/lnbits/database.sqlite3")
        try:
            db.execute(
                "UPDATE accounts SET password_hash = NULL WHERE id = ?", (super_user_id,)
            )
            db.commit()
        finally:
            db.close()
...
def change_btcpay_password(email=None):
    ...
    if result.returncode != 0:
        raise RuntimeError("BTCPay password reset failed: {}".format(result.stderr.strip()))
